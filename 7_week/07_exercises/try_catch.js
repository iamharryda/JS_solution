...
function task1() {
  // Simulate error
  try {
    throw new Error('U made a mistake!!!');
  }
  catch (error) {
...
function task3() {
  ...
  try {
    ...
    console.error("got a bug:", error.message)
  } finally {
...
function task4() {
  ...
  try {
    ...
    try {
      throw new Error("this is the error of inside try")
    } catch (innererror) {
      console.error("inner error:", innererror.message)
    }
    throw new Error("outer error message")
  } catch (outererror) {
    console.error("the outer error:", outererror.message)
  } finally {
...
function task5(array, index) {
  ...
  try {
    if (index < 0 || index > array.length) {
      throw new RangeError("index is not there")
    }
...
function task6(obj, key) {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      console.error("did u make mistake?", error.message)
    }
    else {
