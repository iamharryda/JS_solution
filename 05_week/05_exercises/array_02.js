...
function myAlphabetLength() {
    let len = myAlphabet.length;
    console.log(`The length of myAlphabet: ${len}`)
    if (len >= 5) {
...
for (let i = 0; i < wowDatatypes.length; i++) {
    console.log(`item: ${wowDatatypes[i]}, Index: ${i}, Type: ${typeof (wowDatatypes[i])}`);
}
...
let common = student1Courses.filter(el => student2Courses.includes(el))

console.log(`Common course: ${common[0]}`);
...
furniture.forEach(el => {
    ...
    console.log(`Letters in '${el}': ${letters.slice(0, -2)}`);
});
...
function getPositiveTemperatures() {
    let temp = temperatures.filter(el => el > 0);
    console.log(temp);
}
...
function getOddYears(years) {
    let oddYear = years.filter(year => year % 2 != 0);
    console.log(oddYear);
}
